...
for(i in positions_reasons_for_filter_loss){
  ...
  encounter <- filter_info[i,"encounterId"]
  
  #a while loop to keep moving up while my function says it's ok to do so
...
#im gonna alter the dataframe_length_positions adding columns for starting time, year and quarter that the 
#starting row positions sit in. also rows for the time difference 
...
#this period column is just opasting the year+quarter for ease of graph drawing
...
calculate.quarterly.mean.filter.durations <- function(h){
  ...
  #returning a dataframe for the function 
